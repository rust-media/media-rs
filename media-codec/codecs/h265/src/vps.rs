...
pub struct SubLayerHrdParameters {
    /// Bit rate values minus 1
    pub bit_rate_value_minus1: SmallVec<[u32; MAX_CPB_CNT]>,
    /// CPB size values minus 1
    pub cpb_size_value_minus1: SmallVec<[u32; MAX_CPB_CNT]>,
    /// CPB size DU values minus 1 (if sub_pic_hrd_params_present_flag)
    pub cpb_size_du_value_minus1: SmallVec<[u32; MAX_CPB_CNT]>,
    /// Bit rate DU values minus 1 (if sub_pic_hrd_params_present_flag)
    pub bit_rate_du_value_minus1: SmallVec<[u32; MAX_CPB_CNT]>,
    /// CBR flags
...
impl SubLayerHrdParameters {
    ...
    pub fn parse<R: Read>(reader: &mut BitReader<R, BigEndian>, cpb_cnt: usize, sub_pic_hrd_params_present_flag: bool) -> Result<Self> {
        ...
        params.bit_rate_value_minus1.reserve(cpb_cnt);
        params.cpb_size_value_minus1.reserve(cpb_cnt);
        params.cpb_size_du_value_minus1.reserve(cpb_cnt);
        params.bit_rate_du_value_minus1.reserve(cpb_cnt);
        params.cbr_flag.reserve(cpb_cnt);

        for _ in 0..cpb_cnt {
            params.bit_rate_value_minus1.push(reader.read_ue()?);
            params.cpb_size_value_minus1.push(reader.read_ue()?);
            if sub_pic_hrd_params_present_flag {
                params.cpb_size_du_value_minus1.push(reader.read_ue()?);
                params.bit_rate_du_value_minus1.push(reader.read_ue()?);
            }
...
pub struct HrdParameters {
    ...
    /// AU CPB removal delay length minus 1
    ...
    /// Elemental duration in TC minus 1 per sub-layer
    pub elemental_duration_in_tc_minus1: SmallVec<[u32; MAX_SUB_LAYERS]>,
    /// Low delay HRD flags per sub-layer
    pub low_delay_hrd_flag: SmallVec<[bool; MAX_SUB_LAYERS]>,
    /// CPB count minus 1 per sub-layer
    pub cpb_cnt_minus1: SmallVec<[u32; MAX_SUB_LAYERS]>,
...
impl HrdParameters {
    ...
    pub fn parse<R: Read>(reader: &mut BitReader<R, BigEndian>, common_inf_present_flag: bool, max_num_sub_layers_minus1: u8) -> Result<Self> {
        ...
        if common_inf_present_flag {
            ...
            if hrd.nal_hrd_parameters_present_flag || hrd.vcl_hrd_parameters_present_flag {
                ...
                if hrd.sub_pic_hrd_params_present_flag {
                    hrd.tick_divisor_minus2 = reader.read::<8, u8>()?;
                    hrd.du_cpb_removal_delay_increment_length_minus1 = reader.read::<5, u8>()?;
                    hrd.sub_pic_cpb_params_in_pic_timing_sei_flag = reader.read_bit()?;
                    hrd.dpb_output_delay_du_length_minus1 = reader.read::<5, u8>()?;
                }
                ...
                hrd.initial_cpb_removal_delay_length_minus1 = reader.read::<5, u8>()?;
                hrd.au_cpb_removal_delay_length_minus1 = reader.read::<5, u8>()?;
                hrd.dpb_output_delay_length_minus1 = reader.read::<5, u8>()?;
            }
        }
        ...
        hrd.elemental_duration_in_tc_minus1 = smallvec::smallvec![0; num_sub_layers];
        hrd.low_delay_hrd_flag = smallvec::smallvec![false; num_sub_layers];
        hrd.cpb_cnt_minus1 = smallvec::smallvec![0; num_sub_layers];
        ...
        for i in 0..num_sub_layers {
            ...
            if hrd.fixed_pic_rate_within_cvs_flag[i] {
                hrd.elemental_duration_in_tc_minus1[i] = reader.read_ue()?;
            } else {
                hrd.low_delay_hrd_flag[i] = reader.read_bit()?;
            }

            if !hrd.low_delay_hrd_flag[i] {
                hrd.cpb_cnt_minus1[i] = reader.read_ue()?;
            }

            let cpb_cnt = (hrd.cpb_cnt_minus1[i] + 1) as usize;
    ...
    pub fn au_cpb_removal_delay_length(&self) -> u8 {
        self.au_cpb_removal_delay_length_minus1 + 1
    }
...
pub struct Vps {
    ...
    /// Maximum number of layers minus 1 (0-62)
    ...
    pub vps_max_dec_pic_buffering_minus1: SmallVec<[u32; MAX_SUB_LAYERS]>,
    ...
    /// VPS number of ticks POC diff one minus 1
...
impl Vps {
    ...
    pub fn parse_from_bit_reader<R: Read>(reader: &mut BitReader<R, BigEndian>) -> Result<Self> {
        ...
        let vps_max_layers_minus1 = reader.read::<6, u8>()?;
        // vps_max_sub_layers_minus1 (3 bits)
        let vps_max_sub_layers_minus1 = reader.read::<3, u8>()?;
        if vps_max_sub_layers_minus1 > 6 {
            return Err(invalid_data_error!("vps_max_sub_layers_minus1", vps_max_sub_layers_minus1));
        }
        ...
        let num_sub_layers = (vps_max_sub_layers_minus1 + 1) as usize;

        let mut vps_max_dec_pic_buffering_minus1 = smallvec::smallvec![0u32; num_sub_layers];
        ...
        for i in start_idx..num_sub_layers {
            vps_max_dec_pic_buffering_minus1[i] = reader.read_ue()?;
        ...
        if !vps_sub_layer_ordering_info_present_flag {
            for i in 0..start_idx {
                vps_max_dec_pic_buffering_minus1[i] = vps_max_dec_pic_buffering_minus1[start_idx];
        ...
        let mut layer_id_included_flag = Vec::with_capacity((vps_num_layer_sets_minus1 + 1) as usize);
    ...
    pub fn max_dec_pic_buffering(&self, sub_layer: usize) -> Option<u32> {
        self.vps_max_dec_pic_buffering_minus1.get(sub_layer).map(|v| v + 1)
    }
    ...
    pub fn num_layer_sets(&self) -> u32 {
        self.vps_num_layer_sets_minus1 + 1
    }
}
